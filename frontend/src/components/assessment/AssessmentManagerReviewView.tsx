...
import { Lock, FileQuestion, CheckCircle2, AlertCircle, Eye, Award, HelpCircle } from 'lucide-react';
...
export function AssessmentManagerReviewView({ lessonId }: AssessmentManagerReviewViewProps) {
  ...
  const [assessmentTitle, setAssessmentTitle] = useState('Bài kiểm tra');
    ...
    const fetchAssessment = async () => {
      ...
      try {
        ...
        if (isMounted && managerData && Array.isArray(managerData) && managerData.length > 0) {
          ...
          setAssessmentTitle(firstAss.title || 'Bài kiểm tra');
            ...
            const parsedQuestions: AssessmentQuestion[] = firstAss.questions.map((q: any, idx: number) => {
              ...
              return {
                ...
                options: (q.options || []).map((opt: any, oIdx: number) => ({
                  ...
                  text: opt.content || opt.text || `Phương án ${oIdx + 1}`,
        ...
        try {
          ...
          if (isMounted) setError('Không thể tải danh sách câu hỏi của bài kiểm tra.');
        }
      } finally {
  ...
  if (loading) {
    return (
      <div style={{ padding: '64px 32px', textAlign: 'center', background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 16 }}>
        ...
        <p style={{ fontSize: 14, color: '#6B7280' }}>Đang tải nội dung bài kiểm tra dành cho Quản lý...</p>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: '#9F1239', margin: 0 }}>
              {assessmentTitle} — Chế độ xem dành cho Quản lý
            </h3>
          ...
          <p style={{ fontSize: 13.5, color: '#BE123C', marginTop: 4, marginBottom: 0, lineHeight: 1.5 }}>
            Bạn đang truy cập bài học với quyền Quản lý/Người hướng dẫn. Chức năng nộp bài thi chỉ dành cho học viên. Dưới đây là danh sách chi tiết các câu hỏi và đáp án để kiểm duyệt.
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 12, color: '#6B7280', fontWeight: 500 }}>Tổng số câu hỏi</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#111827' }}>{questions.length} câu</div>
          </div>
        </div>
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 12, color: '#6B7280', fontWeight: 500 }}>Tổng điểm bài thi</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#111827' }}>{totalPoints} điểm</div>
          </div>
        </div>
      </div>
      ...
      {!error && questions.length === 0 ? (
        <div style={{ padding: 48, textAlign: 'center', background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 16 }}>
          ...
          <p style={{ fontSize: 14, color: '#4B5563', fontWeight: 600 }}>Bài kiểm tra chưa có câu hỏi nào</p>
          <p style={{ fontSize: 13, color: '#9CA3AF' }}>Vui lòng thêm câu hỏi ở màn hình quản lý khóa học.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <h4 style={{ fontSize: 15, fontWeight: 700, color: '#111827', margin: 0, paddingLeft: 4 }}>
            Danh sách câu hỏi bài kiểm tra ({questions.length})
          </h4>
                      ...
                      >
                        ...
                        {isCorrect && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#DCFCE7', border: '1px solid #86EFAC', padding: '2px 8px', borderRadius: 6 }}>
                            ...
                            <span style={{ fontSize: 11, fontWeight: 700, color: '#15803D' }}>Đáp án đúng</span>
                          </div>
                        )}
                      </div>
                    );
