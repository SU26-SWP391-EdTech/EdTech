import { Upload, Link2, Bold, Italic, List, Code, CheckCircle2, Plus, Trash2 } from 'lucide-react';
import type { LessonType, QuizQuestion } from '../../../hooks/lesson/useCreateLesson';
...
interface Props {
  ...
  onAddQuiz: () => void; // opens the quiz modal
}

export function LessonContentSection({
  lessonType, videoUrl, setVideoUrl, videoFile, videoUploaded,
  videoInputRef, content, setContent, quizQuestions, setQuizQuestions,
  handleVideoFileChange, onAddQuiz,
}: Props) {
  return (
    <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 14, padding: '22px 26px', marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
        <h2 style={{ fontSize: 14, fontWeight: 700, color: '#111827' }}>Lesson Content</h2>
      </div>
      ...
      {lessonType === 'quiz' && (
        <div>
              ...
              <div key={q.id} style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 10, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <div>
                      <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', color: '#E11D48', background: '#FFF1F3', padding: '2px 6px', borderRadius: 4, marginRight: 8 }}>{q.type}</span>
                      <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.5, marginTop: 4, display: 'inline-block' }}>{q.text}</p>
                    </div>
                  </div>
                  <button onClick={() => setQuizQuestions(prev => prev.filter(item => item.id !== q.id))} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>
          ...
          <button
            onClick={onAddQuiz}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', border: '1.5px dashed #E11D48', borderRadius: 8, background: '#FFF1F3', cursor: 'pointer', fontSize: 13, fontWeight: 600, color: '#E11D48' }}
          >
