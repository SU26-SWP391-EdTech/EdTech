...
import type { DeleteModalProps } from "../../../types/course/my-course.types";

const DeleteModal = ({ deleteId, setDeleteId, onConfirmDelete }: DeleteModalProps) => {
    return (
        deleteId !== null ? (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(4px)' }}>
                <div style={{ background: '#fff', borderRadius: 16, padding: '28px 32px', width: 380, textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                        ...
                        <button onClick={() => onConfirmDelete(deleteId)} style={{ flex: 1, padding: '10px', background: '#E11D48', border: 'none', borderRadius: 9, cursor: 'pointer', fontSize: 13, fontWeight: 600, color: '#fff' }}>Delete</button>
                    </div>
                </div>
            </div>
        ) : null
