...
export function QuizArenaPage({ lessonId, timeLimit, onSubmit }: QuizArenaPageProps) {
        ...
        return (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '300px', fontFamily: "'Inter',sans-serif" }}>
                <div style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ fontSize: 13, color: '#64748B' }}>Đang tạo phòng thi...</p>
    ...
    if (error || questions.length === 0) {
        return (
            <div style={{ padding: 24, textAlign: 'center', color: '#EF4444', fontFamily: "'Inter',sans-serif" }}>
                <p style={{ fontSize: 14 }}>{error || 'Không tìm thấy câu hỏi kiểm tra.'}</p>
            </div>
        );
    }
    ...
    return (
        <div style={{ fontFamily: "'Inter','SF Pro Display',sans-serif", background: '#F8FAFC', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            >
                <QuizTimer timeLeft={timeLeft} totalTime={totalTimeSeconds} title="Kiểm tra cuối khóa" />

                <div style={{ width: 1, height: 20, background: '#E5E7EB' }} />

                {/* Progress bar info */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' }}>
                    <span style={{ fontSize: 12.5, color: '#6B7280' }}>Đã trả lời</span>
            ...
            {showSubmitModal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(17,24,39,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
                    <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 16, padding: '30px 34px', maxWidth: 400, width: '90%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
                        ...
                        <h3 style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 700, color: '#111827' }}>Xác nhận nộp bài?</h3>
                        {answeredCount < TOTAL && (
                            <p style={{ margin: '0 0 10px', fontSize: 13.5, color: '#D97706' }}>
                                Còn <strong>{TOTAL - answeredCount}</strong> câu chưa trả lời sẽ bị tính là sai.
                            </p>
                        )}
                        <p style={{ margin: '0 0 22px', fontSize: 13, color: '#6B7280' }}>
                            Đã trả lời <strong style={{ color: '#111827' }}>{answeredCount}/{TOTAL}</strong> câu. Không thể chỉnh sửa sau khi nộp.
                        </p>
                        <div style={{ display: 'flex', gap: 10 }}>
                            <button onClick={() => setShowSubmitModal(false)} disabled={isSubmitting} style={{ flex: 1, padding: '11px', background: '#F8FAFC', border: '1px solid #E5E7EB', borderRadius: 9, cursor: 'pointer', fontSize: 13.5, color: '#6B7280', fontWeight: 600 }}>Quay lại</button>
                            <button 
                                onClick={submitExam} 
