...
import {
    ...
    FileText, Sparkles, Layers, ChevronDown, BarChart3
} from 'lucide-react';
            ...
            <main className="max-w-[1440px] mx-auto px-8 py-8">
                ...
                <div className="grid grid-cols-12 gap-6">
                    ...
                    <div className="col-span-12 space-y-8">
                        ...
                        {/* Saved Content */}
                        {/* <section>
                            <SectionHeader title="Saved Content" subtitle="8 items bookmarked" actionLabel="Manage saved" />
                            <div className="bg-white border border-[#E5E7EB] rounded-2xl divide-y divide-[#F1F5F9]">
                                {[
                                    { title: 'Advanced TypeScript Patterns', type: 'Course', date: 'Saved 3 days ago', tint: '#FEF2F2', color: '#E11D48' },
                                    { title: 'Full-Stack Engineer Roadmap', type: 'Path', date: 'Saved last week', tint: '#FFFBEB', color: '#F59E0B' },
                                    { title: 'Designing Scalable APIs', type: 'Lesson', date: 'Saved 2 weeks ago', tint: '#EEF2FF', color: '#6366F1' },
                                    { title: 'PostgreSQL Cheat Sheet', type: 'Resource', date: 'Saved last month', tint: '#ECFDF5', color: '#10B981' },
                                ].map((it) => (
                                    <div key={it.title} className="flex items-center gap-4 px-5 py-3.5 hover:bg-[#FAFAFA] transition-colors group">
                                        <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: it.tint, color: it.color }}>
                                            <Bookmark className="w-4 h-4" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm text-[#111827] truncate" style={{ fontWeight: 500 }}>{it.title}</p>
                                            <div className="flex items-center gap-2 mt-0.5">
                                                <span className="px-1.5 py-0.5 text-[10px] rounded" style={{ background: it.tint, color: it.color, fontWeight: 600, letterSpacing: '0.03em', textTransform: 'uppercase' }}>{it.type}</span>
                                                <span className="text-xs text-[#9CA3AF]">{it.date}</span>
                                            </div>
                                        </div>
                                        <button className="p-2 rounded-lg text-[#6B7280] hover:text-[#111827] hover:bg-white opacity-0 group-hover:opacity-100 transition-opacity">
                                            <ArrowUpRight className="w-4 h-4" />
                                        </button>
                                        <button className="p-2 rounded-lg text-[#6B7280] hover:text-[#E11D48] hover:bg-white opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        {/* Learning History */}
                        {/* <section>
                            <SectionHeader title="Learning History" subtitle="Recent activity" actionLabel="View full history" />
                            <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6">
                                <div className="relative">
                                    <div className="absolute left-[15px] top-2 bottom-2 w-px bg-[#E5E7EB]" />
                                    <div className="space-y-5">
                                        <TimelineItem icon={<CheckCircle2 className="w-3.5 h-3.5" />} color="#10B981" tint="#ECFDF5" title="Completed lesson — Routing in React" desc="React Frontend Basics · Lesson 8 of 18" time="2h ago" />
                                        <TimelineItem icon={<BookOpen className="w-3.5 h-3.5" />} color="#E11D48" tint="#FEF2F2" title="Enrolled in Spring Boot REST API" desc="Java Backend Roadmap · 24 lessons" time="Yesterday" />
                                        <TimelineItem icon={<Award className="w-3.5 h-3.5" />} color="#6366F1" tint="#EEF2FF" title="Earned certificate — SQL Database Fundamentals" desc="Issued by DataPath · Verified" time="3 days ago" />
                                        <TimelineItem icon={<Bookmark className="w-3.5 h-3.5" />} color="#F59E0B" tint="#FFFBEB" title="Saved a course — Advanced TypeScript Patterns" desc="Added to your library" time="5 days ago" />
                                        <TimelineItem icon={<Sparkles className="w-3.5 h-3.5" />} color="#8B5CF6" tint="#F5F3FF" title="Finished learning path — DevOps Essentials" desc="6 of 6 courses completed" time="1 week ago" />
                                        <TimelineItem icon={<FileText className="w-3.5 h-3.5" />} color="#0EA5E9" tint="#F0F9FF" title="Submitted a review — Java Basics for Beginners" desc="Rated 5/5 · “Crystal clear explanations”" time="1 week ago" />
                                    </div>
                                </div>
                            </div>
                        </section> */}
                        ...
                        <div className="sticky top-24 space-y-4">
                            ...
                            <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-sm text-[#111827]" style={{ fontWeight: 600 }}>Weekly Goal</h3>
                                    </div>
                                    <button className="text-xs text-[#6B7280] hover:text-[#111827]" style={{ fontWeight: 500 }}>Edit</button>
                                </div>
                                <div className="flex items-center gap-5">
                                    <ProgressRing percent={72} />
                                    <div>
                                        <div className="flex items-baseline gap-1">
                                            <span className="text-[24px] text-[#111827]" style={{ fontWeight: 700 }}>3.6</span>
                                            <span className="text-sm text-[#6B7280]">/ 5h</span>
                                        </div>
                                ...
                                <div className="flex items-center gap-1.5 mt-4">
                                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => (
                                        <div key={i} className="flex-1 text-center">
                                            <div className={`h-7 rounded-md flex items-center justify-center text-[10px] ${i < 5 ? 'bg-[#E11D48] text-white' : 'bg-[#F1F5F9] text-[#9CA3AF]'
                                ...
                                <button className="w-full mt-4 py-2 bg-[#F8FAFC] border border-[#E5E7EB] rounded-lg text-xs text-[#111827] hover:bg-white transition-colors" style={{ fontWeight: 500 }}>
                                    Set study reminder
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-white border border-[#E5E7EB] rounded-2xl p-4">
                                    <div className="flex items-center gap-1.5 mb-2">
                                        ...
                                        <span className="text-[11px] text-[#6B7280]" style={{ fontWeight: 500, letterSpacing: '0.03em', textTransform: 'uppercase' }}>Streak</span>
                                    </div>
                                    <p className="text-[22px] text-[#111827]" style={{ fontWeight: 700 }}>12<span className="text-sm text-[#6B7280] ml-1" style={{ fontWeight: 500 }}>days</span></p>
                                </div>
                                <div className="bg-white border border-[#E5E7EB] rounded-2xl p-4">
                                    <div className="flex items-center gap-1.5 mb-2">
                                        <Clock className="w-3.5 h-3.5 text-[#10B981]" />
                                        <span className="text-[11px] text-[#6B7280]" style={{ fontWeight: 500, letterSpacing: '0.03em', textTransform: 'uppercase' }}>This week</span>
                                    </div>
                                    <p className="text-[22px] text-[#111827]" style={{ fontWeight: 700 }}>3.6<span className="text-sm text-[#6B7280] ml-1" style={{ fontWeight: 500 }}>hrs</span></p>
                                </div>
                            </div>

                            Recommended
                            <div className="bg-gradient-to-br from-[#111827] to-[#1F2937] rounded-2xl p-5 text-white">
                                <div className="flex items-center gap-1.5 mb-3">
                                    <Sparkles className="w-3.5 h-3.5 text-[#F59E0B]" />
                                    <span className="text-[11px] text-white/60" style={{ fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' }}>Recommended next</span>
                                </div>
                                <p className="text-base mb-1" style={{ fontWeight: 600 }}>Designing RESTful Endpoints</p>
                                <p className="text-xs text-white/60 mb-4">Lesson 13 · Spring Boot REST API · 12 min</p>
                                <button className="w-full py-2 bg-white text-[#111827] rounded-lg text-sm hover:bg-white/90 transition-colors flex items-center justify-center gap-2" style={{ fontWeight: 500 }}>
                            ...
                            <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-sm text-[#111827]" style={{ fontWeight: 600 }}>Upcoming Deadlines</h3>
                                    </div>
                                </div>
                                <div className="space-y-3">
                                    {[
                                        { title: 'API Capstone Project', due: 'Due in 3 days', tone: '#E11D48', tint: '#FEF2F2' },
                                        { title: 'React Hooks Quiz', due: 'Due Friday', tone: '#F59E0B', tint: '#FFFBEB' },
                                        { title: 'SQL Final Assessment', due: 'Next week', tone: '#6B7280', tint: '#F8FAFC' },
                                    ].map((d) => (
                                        <div key={d.title} className="flex items-center gap-3">
                                            <div className="w-1 h-8 rounded-full" style={{ background: d.tone }} />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm text-[#111827] truncate" style={{ fontWeight: 500 }}>{d.title}</p>
                                                <p className="text-xs text-[#6B7280]">{d.due}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-sm text-[#111827]" style={{ fontWeight: 600 }}>Achievements</h3>
                                    </div>
                                    <span className="text-xs text-[#E11D48]" style={{ fontWeight: 600 }}>2,480 XP</span>
                                </div>
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <div className="flex-1">
                                        <p className="text-sm text-[#111827]" style={{ fontWeight: 600 }}>Code Marathoner</p>
                                        <p className="text-xs text-[#6B7280]">Earned 2 days ago</p>
                                    </div>
                                </div>
                                <div>
                                    <div className="flex items-center justify-between mb-1.5">
                                        <span className="text-xs text-[#6B7280]" style={{ fontWeight: 500 }}>Next: Backend Pro</span>
                                        <span className="text-xs text-[#111827]" style={{ fontWeight: 600 }}>80%</span>
                                    </div>
                                    <div className="h-1.5 bg-[#F1F5F9] rounded-full overflow-hidden">
                                        <div className="h-full w-4/5 bg-gradient-to-r from-[#8B5CF6] to-[#6366F1] rounded-full" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-[#E5E7EB] rounded-2xl p-5">
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <h3 className="text-sm text-[#111827]" style={{ fontWeight: 600 }}>Skill Progress</h3>
                                </div>
                                <div className="flex flex-wrap gap-1.5">
                                    {[
                                        { name: 'Java', level: 'Advanced', tint: '#FEF2F2', color: '#E11D48' },
                                        { name: 'Spring', level: 'Intermediate', tint: '#FFFBEB', color: '#F59E0B' },
                                        { name: 'SQL', level: 'Expert', tint: '#ECFDF5', color: '#10B981' },
                                        { name: 'React', level: 'Beginner', tint: '#EEF2FF', color: '#6366F1' },
                                        { name: 'TypeScript', level: 'Intermediate', tint: '#F5F3FF', color: '#8B5CF6' },
                                        { name: 'REST APIs', level: 'Advanced', tint: '#F0F9FF', color: '#0EA5E9' },
                                    ].map((s) => (
                                        <span key={s.name} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px]" style={{ background: s.tint, color: s.color, fontWeight: 600 }}>
                                            {s.name}
                                            <span className="text-[10px] opacity-70" style={{ fontWeight: 500 }}>· {s.level}</span>
                                        </span>
                                    ))}
                                </div>
                            </div>
